...
#define MODE 1 //1: Mobile Mode, 2: Joint Mode
...
void CCommWork::_execute() {
	...
	//GET_SYSTEM_MEMORY("Comm1Work_Controller_Target", _target);
	//TO DO
	...
	readSize = _comm.Read((char*)_rvecBuf, 4096);
	...
	for (int i = 0; i < readSize; i++) {
		...
		case 1:
			_packet.buffer[checkSize++] = _rvecBuf[i];
			if (checkSize == 8) {
				mode = 2;
			}
			break;
